...
FRONT_URL = "https://aka5144.github.io/"
BASE_PATH = "TouhouCardGame"  # Base URL configurée dans Vite

# Bordures par rareté
rarityMap = {
    0: None,  # Pas de bordure pour Common
    1: f"{FRONT_URL}/{BASE_PATH}/Assets/Border/bronze.png",
    2: f"{FRONT_URL}/{BASE_PATH}/Assets/Border/silver.png",
    3: f"{FRONT_URL}/{BASE_PATH}/Assets/Border/gold.png",
    4: f"{FRONT_URL}/{BASE_PATH}/Assets/Border/rainbow.png",
}
...
def load_image_from_url(url: str) -> Image.Image:
    print(f"[DEBUG] Trying to load image from: {url}")
...
async def spawn_command(interaction: discord.Interaction):
    ...
    card_url = f"{FRONT_URL}/{BASE_PATH}/{image_url_relative.lstrip('/')}"
...
def register_spawncard_debug(tree: app_commands.CommandTree):
    ...
    async def spawncard_debug(
        ...
        async with pool.acquire() as conn:
            async with conn.cursor() as cursor:
                ...
                if card_row is None:
                    ...
                    return
                _, card_name, image_url_relative, deck_id = card_row
        ...
        card_url = f"{FRONT_URL}/{BASE_PATH}/{image_url_relative.lstrip('/')}"
        print(f"[DEBUG] debug card_url = {card_url}")
